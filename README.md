...
**一个跑在你自己电脑上的免费学术工作台** —— 本科写毕设、硕士做课题、博士攻论文都能用。
14 个面板、零第三方依赖，把「追踪文献 → 转写 PDF → 存摘要卡 → 精读对照 → 记录日志」串成一条流水线。

<br>

**👇 你只需要做一件事：把下面这段发给你手边的 AI Agent**
...
请照着仓库里 SKILL/SKILL.md 的部署向导做：先问我该问的问题，剩下的安装和配置你来做，
装完逐项验证一遍再交给我。请用 main 分支的最新代码，不要下载 Releases 里的历史版本。
```

<div align="center">

**不用会写代码 · 不用打开终端 · 不用自己装 Python · 不用改任何文件**
...
[怎么装](#怎么装) · [它长什么样](#它长什么样) · [面板一览](#面板一览) · [常见问题](#常见问题)
...
> **用的是最新版吗？** 只要让它 `git clone` 这个仓库（或下载 main 分支的 zip），拿到的永远是当前最新代码。
> 仓库主页和 Releases 页各有一个 zip，**别去下 Releases 里标着旧版本的附件**（那是历史存档）。
...
<img src="docs/screenshots/01-overview.jpg" width="900" alt="概览面板：进度卡、今日待办、资讯速览、天气">

> 概览面板。进度卡写的是**你自己的**学段年级，侧栏是**你自己的**研究领域 —— 装完就长这样，不是别人的工具。更多截图见[面板一览](#面板一览)。
...
>   **版本上界 `<4` 必须带上**：MinerU 4.x 重写了命令行接口，本项目调用的是 3.x 的参数，装 4.x 会直接报错（详细原因、版本自检与误装后的补救见下方说明块）。
...
> [!IMPORTANT]
> **给 AI Agent：装 PDF 依赖时请严格执行下面三条。这类问题安装时一声不响，只在用户真去转 PDF 时才报错，所以第 2 条的自检不能省。**
>
> 1. **装本地引擎（版本上界 `<4` 不能省）**
>    `cd pdf_worker && python3 -m venv .venv && .venv/bin/pip install -U "mineru>=3.4,<4"`（Windows 用 `.venv\Scripts\pip`）
>    **不要把 `<4` 去掉、也不要「帮我装最新版」。** MinerU 4.x 把命令行重写成了子命令（`mineru parse <路径> --output …`），本项目调用的是 3.x 的扁平参数，装到 4.x **必定转写失败**。3.x 最后一版是 **3.4.5**，`<4` 正好锁到它。
> 2. **装完立刻验版本**
>    `.venv/bin/pip show mineru | grep -i version` → 必须显示 `3.4.x`。**这一步别跳**：pip 装 4.x 时不会报任何错，用户第一次转 PDF 才会炸。
> 3. **发现已是 4.x 就降回来**（老用户升级、或之前装过最新版的都是这种情况）
>    `cd pdf_worker && .venv/bin/pip install -U "mineru<4"` → 再重启一次服务。

所谓「官网最新版本」在这里**指 3.x 系列内的最新版（3.4.5）**，不是 PyPI 上的 4.x。要装 4.x 得等本项目适配它的新命令行，届时会在这里和 `CHANGELOG.md` 说明。项目代码里已带版本探测：真装成 4.x 时，用户点「转写」会看到中文提示 + 上面第 3 条的修复命令，不会一头雾水。
...
**Q：我完全不懂代码，能装吗？**
能。把开头那段话（[怎么装](#怎么装)）发给你的 AI Agent 就行，它会全程代劳，你只需要回答问题、注册几个免费账号。

**Q：会不会装成旧版本？**
不会。让 Agent `git clone` 这个仓库，或下载 main 分支的 zip，拿到的永远是最新代码。**唯一要避开的是 Releases 页面里的历史版本附件** —— 那里有旧版本的存档包，装了会遇到已知问题。不确定时，让 Agent 检查仓库根目录的 `data/settings.example.json` 里有没有 `degree_level` 字段：有就是新版（v1.1+）。
...
**Q：点转写，提示「本地引擎版本不兼容：检测到已装 MinerU 4.x」？**
这不是坏了，是依赖版本装高了。MinerU 4.x 重写了命令行，本项目用的是 3.x 的参数，所以运行时报错。一行命令降回来即可：

```bash
cd pdf_worker && .venv/bin/pip install -U "mineru<4"   # Windows 用 .venv\Scripts\pip
```

装完重启工作台。以后**升级本地引擎也要带上上界**：`pip install -U "mineru>=3.4,<4"` —— 写 `mineru` 或 `-U mineru` 都会拿到 4.x。这件事直接把上面那句「帮我装一下 PDF 本地引擎」发给你的 AI Agent 也行，它读了仓库的 `SKILL/SKILL.md` 就知道该锁 3.x。

**Q：数据能带走 / 备份吗？**
整个 `data/` 目录就是你的全部数据，复制走即可。

**Q：出了新版本怎么升级？老数据会丢吗？**
不会丢。你的全部数据（配置、日记、待办、转写记录、每日快照）都在 `data/` 与 `data_snapshots/` 里，**升级只替换代码，这两个文件夹不动**。最省事的做法——把这句话发给你的 AI Agent：

```text
帮我升级学术工作台到最新版：先备份整个文件夹，再更新代码（data 文件夹千万别动），
启动后按 SKILL/SKILL.md 第 5 步自检表验证一遍，确认我的日记和待办都还在。
```

手动升级两条路：git 安装的（文件夹里有 `.git`）进文件夹执行 `git pull`；zip 安装的重新下载 main 最新包解压，把旧文件夹里的 `data/` 整体拷进新文件夹。详细流程见 `SKILL/SKILL.md` 的「老用户升级」一节。
注意：升级后如果进度卡显示"待设置"，**不是数据丢了**——是新版新增了配置项（如学段 / 学制），按提示补填即可，原有记录都在。
